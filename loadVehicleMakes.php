...
$sql = "SELECT * FROM doli_auto_makes ORDER BY type ASC";
...
if ($response)
{
	...
	if ($num)
	{
        while ($i < $num)
		{
			$obj = $db->fetch_object($vehicleMakes);
			if ($obj)
			{
						$vehicleMakes[$i] = $obj;
			}
		...
		$vehicleMakes = "FAILURE: SQL Returned ".$num." results";
	}
}
else{
	$vehicleMakes = "FAILURE: SQL Request Failed";
}

print json_encode($vehicleMakes);
